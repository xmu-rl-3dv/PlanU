...
class Evaluator():
    ...
    def evaluate(self,
        ...
        success_prob = os.environ.get("SUCCESS_PROBABILITY","")

        version_steps = f"{version}_{steps}" if version and steps else "default"
        ...
        for i, example in enumerate(tqdm(self.dataset,
            ...
            # final_state = algo_output.trace[0][-1].blocks_state

            if algo_output.trace and len(algo_output.trace) > 0 and len(algo_output.trace[0]) > 0:
                last_node = algo_output.trace[0][-1]
                final_state = last_node.blocks_state if last_node.blocks_state is not None else last_node.last_blocks_state
                goal = example['goal']
                # correct = self.eval_stochastic_ouput(final_state, goal)
                # correct = self.eval_output(answer, output)
                correct = self.eval_output_new(final_state, goal)
            else:
                print("Trace is empty or invalid.")
                final_state = None
                correct = False
