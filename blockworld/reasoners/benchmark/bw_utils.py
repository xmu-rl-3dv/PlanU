...
def validate_plan_new(answer, output):
    """
    Validate if the given output matches the expected plan in the answer.

    :param answer: dict, containing keys 'plan' (expected plan) and other metadata.
    :param output: str, the actual plan to validate.
    :return: bool, True if the output matches the expected plan, False otherwise.
    """
    # Extract the expected plan from the answer and clean it up
    expected_plan = answer.get("plan", "").strip()
    # Clean up the provided output for comparison
    provided_output = output.strip()

    # Remove markers like [PLAN END] from both the expected and provided plans for a fair comparison
    expected_plan_cleaned = expected_plan.replace("[PLAN END]", "").strip()
    provided_output_cleaned = provided_output.replace("[PLAN END]", "").strip()

    #
    # is_correct = expected_plan_cleaned == provided_output_cleaned
    # 由完全一致改为包含关系
    is_correct = expected_plan_cleaned in provided_output_cleaned
...
def generate_all_actions(state):
    ...
    if "hand is empty" in state:
        # 提取所有"clear"的方块颜色
        blocks = re.findall(r"the ([a-z]{0,10}) block is clear", state)
        for c in blocks:
            # 检查方块是否在桌上
            if f"the {c} block is on the table" in state:
                return_list.append(f"pick up the {c} block")
        ...
        # 提取当前手持方块颜色
        match = re.search(r"is holding the ([a-z]{0,10}) block", state)
        if match:
            c = match.group(1)
            # 提取所有可堆叠的clear方块
            clear_blocks = re.findall(r"the ([a-z]{0,10}) block is clear", state)
            for c_ in clear_blocks:
                return_list.append(f"stack the {c} block on top of the {c_} block")
            return_list.append(f"put down the {c} block")
    return return_list
