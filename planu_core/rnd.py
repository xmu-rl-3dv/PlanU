...
class RndRewardModel(BaseRewardModel):
    ...
    def __init__(self, config: EasyDict, device: str = 'cuda', tb_logger: 'SummaryWriter' = None) -> None:  # noqa
    ...
    def estimate(self, data: list) -> List:
        """
        ...
        obs = torch.tensor(data,device=self.device).view(-1)
        # obs = torch.stack(data).to(self.device)
        if self.cfg.obs_norm:
            # Note: observation normalization: transform obs to mean 0, std 1
        ...
        with torch.no_grad():
            ...
            mse = F.mse_loss(predict_feature, target_feature, reduction='none').mean()
