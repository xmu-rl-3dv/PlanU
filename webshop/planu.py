...
WEBSHOP_URL = "http://172.17.0.3:3001/"
ACTION_TO_TEMPLATE = {
...
class webshopEnv:
  ...
  def step(self, session, action, env_state=None): # 新增env_state，用于动作失败时需要重新加载sessions
    done = False
    observation_ = None
    self.sessions = env_state if env_state is not None else self.sessions
...
class Node:
    def __init__(self, state, question, env_state=None, parent=None, node_type='state'):
        ...
        self.quantile_values = np.ones(self.n_atoms) * 0.5
        self.quantile_probs = np.linspace(1/(2*self.n_atoms), 1-1/(2*self.n_atoms), self.n_atoms)
    ...
    @property
    def ucc_value(self,):
     return np.sum(self.quantile_values/len(self.quantile_probs))
...
def planu_search(args, task, idx, iterations=50, to_print=True):
    global gpt
    global failed_trajectories
    global reflection_map
    global failure_times
    action = 'reset'
    ...
    for i in range(iterations):
        ...
        if node.is_terminal and node.reward == 1:
            ...
            return node.state, node.value, all_nodes, node.reward, node.em, failure_times
        ...
        if len(node.children)>0:
            terminal_node = rollout(max(node.children, key=lambda child: child.ucc_value), args, task, idx, max_depth=20)
        ...
        if terminal_node.reward == 1:
            ...
            logging.info(f"Terminal node with reward 1 found at iteration {i + 1}")
            return terminal_node.state, terminal_node.value, terminal_node.reward, terminal_node.em, failure_times
        ...
        if terminal_nodes_with_reward_1:
            ...
            return best_node.state, best_node.value, best_node.reward, best_node.em, failure_times
    ...
    return best_child.state, best_child.ucc_value, best_child.reward, best_child.em, failure_times

def simple_search(args, task, idx, iterations=8, max_depth=20, to_print=True):
    # Initialization
    action = 'reset'
    global failed_trajectories
    global reflection_map
...
def rollout(node, args, task, idx, max_depth=20):
    global failure_times
    depth = 0
    n = 5
    delay = sample_latency_check(200)
    action_line = node.state['action']
    if not delay and action_line != 'click[buy now]' and 'search' not in action_line and failure_times<=2000:
        failure_times += 1
...
def select_node(node):
    while node and node.children:
        ...
        node = max((child for child in node.children if not child.is_terminal), key=lambda child: child.ucc_value, default=None)
        while node.is_terminal and node.reward != 1:
            ...
            node = max((child for child in node.children if not child.is_terminal), key=lambda child: child.ucc_value, default=None)
            delay = sample_latency_check(200)
            if not delay and action_line != 'click[buy now]' and ('click[b' in action_line or 'search' in action_line) and failure_times<=2000:
                failure_times += 1
                return node.children[1]
            else:
                return node.children[0]
            
        logging.info(f"Selected node at depth {node.depth} with UCC {node.ucc_value}.")
...
def generate_new_states(node, args, task, idx, n):
    ...
    assert len(sampled_actions) > 0, "No actions sampled!"
    ...
    for action in sampled_actions:
        ...
        if action_line in list(unique_states.keys()):
            continue
        ...
        delay = sample_latency_check(200)
        env_state_copy = node.env_state.copy()
        if action_line:
            try:
                res = env.step(idx, action_line, env_state=env_state_copy)
                #print("res", res)
                obs = res[0]
                r = res[1]
                done = res[2]
            ...
            new_node = Node(state=new_state, question=node.question, env_state=env_state_clone, parent=node, node_type = 'action')
            new_node_children_fail = Node(state=node.state, question=node.question, env_state=local_sessions, parent=new_node, node_type = 'state')
            new_node_children_suc = Node(state=new_state, question=node.question, env_state=env_state_clone, parent=new_node, node_type = 'state')
            new_node.children.append(new_node_children_suc)
            new_node.children.append(new_node_children_fail)
            # new_node.env_state = local_sessions
            if r > 0 or done:
                logging.info(f"reward:{r}")
                new_node.children[0].is_terminal = True
                #print("rew", r)
            new_node.children[0].reward = r
            new_node.children[0].value = r
    ...
    node.children.extend(list(unique_states.values()))
...
def backpropagate(node, value):
    ...
    def compute_cum_reward(rewards):
        return sum([r * (discount ** i) for i, r in enumerate(rewards)])
    while node:
        # cum_reward = compute_cum_reward(value[-j-1:])
        cum_reward = value
        ...
        if node.state is not None:
            ...
            for i, tau in enumerate(node.quantile_probs):
                ...
                node.quantile_values[i] += alpha * gradient * abs(delta)
