...
import virtual_home
from mcts.virtualhome.PlanU_v2 import LLMAgent, LanguageNode, ActionNode
from datetime import datetime
import logging
import json
from torch.utils.tensorboard import SummaryWriter
log_dir = 'log/entertainment/mcts+'
...
log_file = os.path.join(log_dir, f'stochastc={stochastic}_vw={value_weight}_{time_str}.txt')
...
def main():
    ...
    parser.add_argument('--temperature', type=float, default=1, action= "store",help='temperature')
    ...
    writer = SummaryWriter(f"./results/Model={args.base_model}/entertainment/PlanU_grab_temperature/seed={args.seed}/stochastic={args.stochastic}/rnd={args.rnd}")
    writer.add_text(
        "hyperparameters",
        "|param|value|\n|-|-|\n%s" % ("\n".join([f"|{key}|{value}|" for key, value in vars(args).items()])),
    )
    rnd_writer = SummaryWriter(f"./rnd_results/Model={args.base_model}/entertainment/mctspp/seed={args.seed}/stochastic={args.stochastic}/rnd={args.rnd}")
    ...
    agent = LLMAgent(normalization_mode="word",rnd = args.rnd,tb_logger=rnd_writer,base_model = args.base_model, temperature = args.temperature
                     ) # word
    ...
    step_list = []
    obs = envs.reset()
    root = LanguageNode(state=obs,initial_value = torch.tensor([1],device=device))
    ...
    for i in range(args.maxiterations):  #100
        ...
        while not done and steps < 15:
            ...
            if args.stochastic>0 and 'grab' in action_name and random.random()<args.stochastic: # state transfer uncertainty. Stochastic situation
                ...
                # stoc = False
            print("action", action, 'action name', action_name,' reward:', reward)
            logging.info(f"action : {action_name}  reward : {reward*discount}")
            if args.rnd:
        ...
        if args.rnd and train_data_cnt > 15:
            agent.train()
        print("episode:",i,"step:",steps, "return:",rewards)
        logging.info(f"number:",i,"steps : {steps}, rewards : {rewards}")
        writer.add_scalar("charts/episodic_return", rewards, i)
        if rewards>=1:
            num_success+=1
    print("num_success",num_success)
    print('tokens:',agent.total_llm_tokenizer_token)
    ...
    with open("token_consumed.txt", "a") as f:
        f.write("--------------------------------------\n")
        f.write(f"./results_new/Model={args.base_model}/en/PlanU/seed={args.seed}/stochastic={args.stochastic}/rnd={args.rnd}")
