...
def is_success(episodic_return: float) -> bool:
    return episodic_return >= 1.0


def _build_run_paths(args, config_digest):
    result_path = (
        f"./results/Model={args.base_model}/entertainment/"
        f"PlanU_grab_temperature/seed={args.seed}/"
        f"stochastic={args.stochastic}/rnd={args.rnd}/"
        f"config={config_digest}"
    )
    rnd_path = (
        f"./rnd_results/Model={args.base_model}/entertainment/mctspp/"
...
def _token_log_path(args, config_digest):
    return (
        f"./results_new/Model={args.base_model}/en/PlanU/"
        f"seed={args.seed}/stochastic={args.stochastic}/rnd={args.rnd}/"
        f"config={config_digest}"
    )
...
def run(args) -> None:
    ...
    try:
        ...
        consumed_tokens = scorer.total_llm_tokenizer_token
        query_times = scorer.total_llm_tokenizer_call
