...
class Node(object):
    ...
    def __init__(
        ...
        calc_q: Callable[[list[float]], float] = np.mean,n_atoms: int = 51, v_min: float = -1, v_max: float = 1
    ) -> None:
        self._parent = parent
        self._children = {}
        self._visit_count = 0
...
class ActionNode(Node):
    ...
    def trans_v(self,):
        ...
        def trans_v(self):
            """
            Calculate the average transition value.
            
            Returns:
                float: Average transition value (total value / visit count), or 0 if never visited
            """
...
class LLMAgent:
    ...
    def select(self, obs, node, global_step):
        ...
        if self.rnd:
            ...
            ex_reward = 0.5 * torch.div(ex_reward, max(1, len(node.cum_rewards)))
        ...
        if self.task == 0 and (global_step < 50 or (global_step<100 and random.random()<0.2)):
            return torch.tensor([sampled_index]),values[sampled_index], node.children[action[sampled_index]], action[sampled_index]
        else:
            return torch.tensor([argmax]),values[argmax], node.children[action[argmax]],action[argmax]
    ...
    def expand(self, obs, node, simulate_envs, if_action_node=False, value_weight = 0.5):
        ...
        if not LLM:
            ...
            for id, action in enumerate(action_list):
                ...
                n = np.array(reward+1)
                re = torch.tensor(n,device=self.device).view(-1)
                child = ActionNode(parent=node, state = next_obs, last_action=action, initial_value = re,value_weight=value_weight) #修改，添加了reward
        ...
        with torch.no_grad():
            outputs = self.model(input_ids, attention_mask=attention_mask)
        self.total_llm_tokenizer_token += len(outputs.logits) * len(outputs.logits[0])
        ...
        for id, action in enumerate(action_list):
            ...
            n = np.array(reward)
            re = torch.tensor(n,device=self.device).view(-1)
            child = ActionNode(parent=node, state = next_obs, last_action=action, initial_value = probs.log_prob(id_tmp).exp()+ re,value_weight=value_weight) #修改，添加了reward
    ...
    def obs2text(self, obs):
        if self.task == 3:
            ...
            sentences = ["There are two fixed cutting boards in the room."]
            if prompt_disturb:
                sentences = ["There are two fixed cutting boards in the room. Earlier this day, you made a dish with chopped potatoes."]
                # print(sentences)
            ...
            if cindex >= 0:
                ...
                if len(overlay["in_agent"]) == 0:
                    ...
                    action_list[5] = action_template.format("bowl", "second")
            # sentences.append("To serve the dish of a bowl only containing chopped tomato and lettuce, you should first")
            sentences.append("Your goal is to serve the dish of a bowl only containing chopped tomato and lettuce.")
            ...
            # sentences.append("To serve the dish of a bowl only containing chopped tomato, you should first")
            sentences.append("Your goal is to serve the dish of a bowl only containing chopped tomato.")
            sentences.append("Your next step is to")
        if prompt_shuffle:
            temp = sentences[:-1]
            random.shuffle(temp)
